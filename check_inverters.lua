...
local MIN_WH = 550                -- Min average WhToday to check status
    ...
    execute = function(domoticz, item)
        ...
        then
            ...
            do
                ...
                then
                    local msg = string.format("Inverter '%s' issue: Wh today=%.0f / Mean Wh today=%.0f / diff Wh=%.0f (%.2f%%)", device.name, device.WhToday, mean_wh, diff, diff_percent)
                    domoticz.log(msg, domoticz.LOG_DEBUG)
                    domoticz.notify("Inverter '" .. device.name .. "' issue", msg, domoticz.PRIORITY_HIGH, nil, nil, nil)
                else
                    local msg = string.format("Inverter '%s' within range: Wh today=%.0f / Mean Wh today=%.0f / diff Wh=%.0f (%.2f%%)", device.name, device.WhToday, mean_wh, diff, diff_percent)
                    domoticz.log(msg, domoticz.LOG_DEBUG)
                end
            end
        end
